...
#define APPEND(DATA)  	NodePtr new_node=AlNode_traits::allocate(this->get_val(),1);\
						/* AlNode_traits::construct */CONSTRUCT(this->get_val(),new_node,DATA);\
...
#define POP_BACK		NodePtr last=tail;\
						...
						/* AlNode_traits::destroy */ DESTROY(this->get_val(),last);\
	...
	class List:EBCO<rebind_alloc<Alloc,List_Node<T,typename allocator_traits<Alloc>::void_pointer>>> COUNT_IT_{
		...
		private:
			struct AuxList{
				...
				AuxList(const List& list,AlNode& alloc){
					...
					if(p!=nullptr){
						...
						/* AlNode_traits::construct */CONSTRUCT(alloc,this->head,p->data);
						this->tail=this->head;
						p=p->next;
						while(p!=nullptr){
							this->tail->next=AlNode_traits::allocate(alloc,1);
							/* AlNode_traits::construct */CONSTRUCT(alloc,this->tail->next,p->data);
							this->tail->next->prev=this->tail;
							this->tail=this->tail->next;
							p=p->next;
						}
				...
				AuxList(List&& list,AlNode& alloc){
					if(list.head!=nullptr){
						this->head=AlNode_traits::allocate(alloc,1);
						/* AlNode_traits::construct */CONSTRUCT(alloc,this->head,list.head->data);
						this->tail=this->head;
						...
						while(list.head!=nullptr){
							this->tail->next=AlNode_traits::allocate(alloc,1);
							/* AlNode_traits::construct */CONSTRUCT(alloc,this->tail->next,list.head->data);
							this->tail->next->prev=this->tail;
							this->tail=this->tail->next;
							aux=list.head;
							list.head=list.head->next;
			...
			inline void cut_alloc_helper(NodePtr start,AlNode& dealloc){
				if(start!=nullptr){
					...
					NodePtr aux=start;
					start=start->next;
					/* AlNode_traits::destroy */DESTROY(dealloc,aux);
		...
		public:
			...
			List cut(size_type start,size_type end){
				...
				}// no va un else porque puede que siga de largo si el if(new_list.get_val()!=this->get_val()) no se cumple
