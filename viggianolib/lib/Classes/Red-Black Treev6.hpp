...
namespace mpv{
    ...
    struct Base_Tree_Node{
        ...
        BaseNodePtr prev=nullptr;
        BaseNodePtr left=nullptr;
        BaseNodePtr right=nullptr;
        RedOrBlack color=BLACK;
        constexpr BaseNodePtr address()const noexcept{
            return pointer_traits<BaseNodePtr>::pointer_to(*const_cast<Base_Tree_Node*>(this));
        }
        constexpr void reset()noexcept{
            prev=left=right=nullptr;
            color=RED;
        }
        constexpr BaseNodePtr maximum()const noexcept{
            BaseNodePtr p=this->address();
            while(p->right)
                p=p->right;
            return p;
        }
        constexpr BaseNodePtr minimum()const noexcept{
            BaseNodePtr p=this->address();
            while(p->left)
                p=p->left;
            return p;
    ...
    template<typename> class const_Tree_iterator;
    template<typename Types>
    class Tree_iterator{//in order
        template<typename> friend class Tree;
        friend class const_Tree_iterator<Types>;
        using const_iterator=const_Tree_iterator<Types>;
		public:
    ...
    class const_Tree_iterator{//in order
        template<typename> friend class Tree;
        friend class Tree_iterator<Types>;
		public:
            using iterator_category=bidirectional_iterator_tag;
			using Node=typename Types::Node;
            using BaseNodePtr=typename Types::BaseNodePtr;
    ...
    class Tree COUNT_IT{
		...
		protected:
            struct handle_value_compare:private EBCO<value_compare>{
                constexpr handle_value_compare(key_compare cmp)noexcept(is_nothrow_constructible_v<value_compare,key_compare&&>):EBCO<value_compare>(mpv::move(cmp)){}
                constexpr bool operator()(const node_type& fst,const value_type& scnd)const noexcept(noexcept(fake_copy_init<bool>(this->get_val()(fst.value(),scnd)))){
                    return this->get_val()(fst.value(),scnd);
                }
                constexpr bool operator()(const value_type& fst,const node_type& scnd)const noexcept(noexcept(fake_copy_init<bool>(this->get_val()(fst,scnd.value())))){
                    return this->get_val()(fst,scnd.value());
                }
            };
            struct NodeDeleteGuard{
                NodeDeleteGuard(const NodeDeleteGuard&)=delete;
                NodeDeleteGuard& operator=(const NodeDeleteGuard&)=delete;
                AlNode& al;
                NodePtr node;
                constexpr NodeDeleteGuard(AlNode& al,NodePtr node)noexcept:al(al),node(node){}
                ~NodeDeleteGuard(){
				    DESTROY(al,node);
				    AlNode_traits::deallocate(al,node,1);
                }
            };
...
#define sent        ct.getV3().address()
			...
			constexpr static const_NodePtr to_cnode(BaseNodePtr bp)noexcept{
				return pointer_traits<const_NodePtr>::pointer_to(static_cast<Node&>(*bp));
			}
            ...
            constexpr static bool is_red(BaseNodePtr p)noexcept{
                return p!=nullptr && p->color==RED;
            }
            constexpr static bool is_black(BaseNodePtr p)noexcept{
                return p==nullptr || p->color==BLACK;
            }
            constexpr bool is_null(BaseNodePtr p)const noexcept{
            ...
            constexpr bool cmp(const key_type& a,const key_type& b)const noexcept(noexcept(fake_copy_init<bool>(this->ct.getV1()(a,b)))){
            	return ct.getV1()(a,b);
			}
			template<typename... Args>
            ...
            constexpr BaseNodePtr create_node(node_type&& handle)noexcept(ALWAYS_EQ){
                if constexpr(ALWAYS_EQ) return to_base(handle.drop());
                else{
                    if(alloc==handle.node_alloc) return to_base(handle.drop());
                    else{
                        BaseNodePtr ret=create_node(static_cast<value_type&&>(handle.value()));
                        handle.destroy();
                        return ret;
                    } 
                }
            ...
            constexpr void clr(BaseNodePtr node)noexcept{
                if(is_null(node)) return;
                else{
                    clr(node->left);
                    clr(node->right);
                    delete_node(to_node(node));
                }
            }
            template<typename Lambda>
            constexpr void pre_order_helper(BaseNodePtr node,Lambda& func)const noexcept(noexcept(func(to_cnode(node)->data))){
                if(is_null(node)) return;
                func(to_cnode(node)->data);
                pre_order_helper(node->left,func);
                pre_order_helper(node->right,func);
            }
            template<typename Lambda>
            constexpr void in_order_helper(BaseNodePtr node,Lambda& func)const noexcept(noexcept(func(to_cnode(node)->data))){
                if(is_null(node)) return;
            ...
            template<typename Lambda>
            constexpr void post_order_helper(BaseNodePtr node,Lambda& func)const noexcept(noexcept(func(to_cnode(node)->data))){
                if(is_null(node)) return;
                post_order_helper(node->left,func);
                post_order_helper(node->right,func);
                func(to_cnode(node)->data);
            }
            constexpr void insert_fixup(BaseNodePtr x)noexcept{
                while(is_red(x->prev)){
            ...
            constexpr void delete_fixup(BaseNodePtr x)noexcept{
                while(x!=root && x->color==BLACK){
                    if(x==x->prev->left){
                        BaseNodePtr w=x->prev->right;
                        if(is_red(w)){
                            w->color=BLACK;
                            x->prev->color=RED;
                            left_rotate(x->prev);
            ...
            constexpr void right_rotate(BaseNodePtr x)noexcept{
                BaseNodePtr y=x->left;
                if(x->prev==nullptr)
                    this->root=y;
                else{
                    if(x==x->prev->right)
            ...
            constexpr void left_rotate(BaseNodePtr x)noexcept{
                BaseNodePtr y=x->right;
                if(x->prev==nullptr)
                    this->root=y;	
                else{
                    if(x==x->prev->left)
            ...
            constexpr void transplant(BaseNodePtr u,BaseNodePtr v)noexcept{//Desenlaza u con su padre y enlaza al padre con v
                if(u->prev==nullptr)
                    this->root=v;
            ...
            template<typename Lambda>
            constexpr void iterative_in_order(Lambda&& func=Lambda{})noexcept(noexcept(func(to_node(root)->data))){
                if(root==sent) return;
                BaseNodePtr ptr=left_most;
                while(true){
                    func(to_node(ptr)->data);
                    if(ptr->right!=nullptr){
            ...
            template<typename Lambda>
            constexpr void iterative_in_order(Lambda&& func=Lambda{})const noexcept(noexcept(func(to_cnode(root)->data))){
                if(root==sent) return;
                BaseNodePtr ptr=left_most;
                while(true){
                    func(to_cnode(ptr)->data);
                    if(ptr->right!=nullptr){
                        if(ptr==ptr->right->left) return;
                        else ptr=ptr->right->minimum();
                    }
            ...
            constexpr iterator insert_without_repeated(val_type&& val){//universal reference of value_type
                ...
                if(root==sent){
                ...
                else if(!vcmp(val,to_node(right_most)->data)){//val>=right_most->data
                    if(vcmp(to_node(right_most)->data,val)){//val>right_most->data
                ...
                else if(!vcmp(to_node(left_most)->data,val)){//val<=left_most->data
                    if(vcmp(val,to_node(left_most)->data)){//val<left_most->data
                    ...
                    while(true){
                        ...
                        else if(vcmp(to_node(x)->data,val)){
                            if(x->right==nullptr){
            ...
            constexpr iterator insert_with_repeated(val_type&& val){//universal reference of value_type
                ...
                else if(vcmp(val,to_node(left_most)->data)){//val<left_most->data
                    left_most->left=create_node(static_cast<val_type&&>(val));
                    left_most->left->prev=left_most;
                    left_most=left_most->left;
                    left_most->left=sent;
            ...
            struct TreeDeleterFunc{
                constexpr void operator()(Tree& tree){
                    tree.iterative_deletion();
                }
            };
        public:
            constexpr Tree()noexcept(is_nothrow_default_constructible_v<AlNode> && is_nothrow_default_constructible_v<key_compare>){root=left_most=right_most=sent;}
            ...
            constexpr explicit Tree(const key_compare& comp,const Alloc& al=Alloc{})noexcept(is_nothrow_copy_constructible_v<key_compare>):ct(arg1and2_tag{},comp,al){root=left_most=right_most=sent;}
            ...
            constexpr Tree(It first,It last,const key_compare& comp=key_compare{},const Alloc& al=Alloc{}):ct(arg1and2_tag{},comp,al){
                root=left_most=right_most=sent;
                TreeDeleterFunc d;
                TemporaryOwnerDel<Tree&,TreeDeleterFunc> guard(*this,d);
                while(first!=last) insert(*first++);
                guard.call_deleter=false;
            }
            ...
            constexpr Tree(Tree&& other)noexcept(is_nothrow_move_constructible_v<key_compare>):ct(arg1and2_tag{},static_cast<key_compare&&>(other.ct.getV1()),static_cast<AlNode&&>(other.alloc)),length(other.length){
            ...
            constexpr Tree(Tree&& other,const Alloc& al)noexcept(ALWAYS_EQ && is_nothrow_move_constructible_v<key_compare>):ct(arg1and2_tag{},static_cast<key_compare&&>(other.ct.getV1()),al){
            ...
            constexpr Tree& operator=(Tree&& other)noexcept((ALWAYS_EQ || POCMA) && is_nothrow_move_assignable_v<key_compare>){
                if(this==&other) return *this;
                this->clear();
            ...
            constexpr iterator insert(node_type&& handle)noexcept(ALWAYS_EQ && noexcept(handle_value_compare(this->ct.getV1())) && noexcept(fake_copy_init<bool>(declval<const value_compare&>()(mpv::declval<const value_type&>(),mpv::declval<const value_type&>())))){// If allocator is always equal this function won't allocate memory
            ...
            constexpr bool del(const_iterator pos)noexcept{//pos should be an iterator to an element of this or the end() iterator.
                ...
                BaseNodePtr z=pos.ptr;//the node that will be deleted
                if(z==root && z->left==sent && z->right==sent){
                    delete_node(to_node(z));
                    reset();
                    return true;
                }
                BaseNodePtr x;//the node that repaces z
            ...
            constexpr node_type extract(const_iterator pos)noexcept{//pos should be an iterator to an element of this or the end() iterator.
                if(pos==end()) return {};
                BaseNodePtr z=pos.ptr;
                if(z==root && z->left==sent && z->right==sent){
                    z->reset();
                    reset();
                    return node_type(to_node(z),alloc);
                }
                BaseNodePtr x;//the node that repaces z
                RedOrBlack original_color=z->color;
                TempNode tempnode;
            ...
            constexpr node_type extract(const key_type& val)noexcept(noexcept(this->find(val))){
            ...
            constexpr Optional<value_type> pop(const_iterator pos)noexcept(is_nothrow_move_constructible_v<value_type>){
                ...
                BaseNodePtr z=pos.ptr;//the node that will be deleted
                if(z==root && z->left==sent && z->right==sent){
                    NodeDeleteGuard guard(alloc,to_node(z));
                    reset();
                    return static_cast<value_type&&>(to_node(z)->data);
                }
                BaseNodePtr x;//the node that repaces z
                RedOrBlack original_color=z->color;
                TempNode tempnode;
            ...
            constexpr bool del_elem(const key_type& val)noexcept(noexcept(this->find(val))){
                return del(find(val));
            }
            constexpr Optional<value_type> pop_elem(const key_type& val)noexcept(noexcept(this->pop(this->find(val)))){
                return pop(find(val));
            }
            ...
            constexpr bool contains(const key_type& val)const noexcept(noexcept(this->cmp(val,val))){
                if(root==sent || cmp(Traits::getKey(to_node(right_most)->data),val) || cmp(val,Traits::getKey(to_node(left_most)->data))) return false;
                else if((!cmp(val,Traits::getKey(to_node(right_most)->data)) && !cmp(Traits::getKey(to_node(right_most)->data),val)) || (!cmp(val,Traits::getKey(to_node(left_most)->data)) && !cmp(Traits::getKey(to_node(left_most)->data),val)))/*val==right_most->data || val==left_most->data*/ return true;
                BaseNodePtr x=root;
                while(true){
                    if(cmp(val,Traits::getKey(to_node(x)->data))){
                        if(x->left==nullptr)
                            return false;						
                        else
                            x=x->left;
                    }
                    else if(cmp(Traits::getKey(to_node(x)->data),val)){
                        if(x->right==nullptr)
                            return false;							
                        else
                            x=x->right;
            ...
            constexpr iterator find(const key_type& val)noexcept(noexcept(this->cmp(val,val))){
                if(root==sent || cmp(Traits::getKey(to_node(right_most)->data),val) || cmp(val,Traits::getKey(to_node(left_most)->data))) return end();
                if constexpr(!allow_repeated){
                    if(!cmp(val,Traits::getKey(to_node(right_most)->data)) && !cmp(Traits::getKey(to_node(right_most)->data),val))/*val==right_most->data*/ return iterator(right_most);
                    else if(!cmp(val,Traits::getKey(to_node(left_most)->data)) && !cmp(Traits::getKey(to_node(left_most)->data),val))/*val==left_most->data*/ return iterator(left_most);
                }
                BaseNodePtr x=root;
/*  Aclaracion
...
*/
            ...
            constexpr const_iterator find(const key_type& val)const noexcept(noexcept(this->cmp(val,val))){
                if(root==sent || cmp(Traits::getKey(to_node(right_most)->data),val) || cmp(val,Traits::getKey(to_node(left_most)->data))) return end();
                if constexpr(!allow_repeated){
                    if(!cmp(val,Traits::getKey(to_node(right_most)->data)) && !cmp(Traits::getKey(to_node(right_most)->data),val))/*val==right_most->data*/ return iterator(right_most);
                    else if(!cmp(val,Traits::getKey(to_node(left_most)->data)) && !cmp(Traits::getKey(to_node(left_most)->data),val))/*val==left_most->data*/ return iterator(left_most);
            ...
            constexpr enable_if_t<enable,size_type> count(const key_type& val)const noexcept(noexcept(this->cmp(val,val))){
			...
			template<typename Lambda>
			constexpr void foreach(Lambda&& func=Lambda{})const noexcept(noexcept(this->iterative_in_order(func))){
            ...
            template<typename Lambda>
            constexpr void pre_order(Lambda&& func=Lambda{})const noexcept(noexcept(pre_order_helper(root,func))){
                pre_order_helper(root,func);
            }
            template<typename Lambda>
            constexpr void in_order(Lambda&& func=Lambda{})const noexcept(noexcept(in_order_helper(root,func))){
                in_order_helper(root,func);
            }
            template<typename Lambda>
            constexpr void post_order(Lambda&& func=Lambda{})const noexcept(noexcept(post_order_helper(root,func))){
                post_order_helper(root,func);
            }
			constexpr bool operator==(const Tree& other)const noexcept(noexcept(equal(this->begin(),this->end(),other.begin()))){
            ...
            constexpr bool operator<(const Tree& other)const noexcept(noexcept(fake_copy_init<bool>(*this->begin()<*this->begin()))){
            ...
            constexpr const key_compare& get_key_cmp()const noexcept{
                return ct.getV1();
            }
