...
namespace mpv {
	...
	template<typename T, size_t N>
	class uPtr<T[N]> COUNT_IT{
	private:
		T* data;
	public:
		uPtr(const uPtr&) = delete;
		uPtr<T[N]>& operator=(const uPtr&) = delete;
		uPtr() :data(nullptr) {
			INCUNIQUEPOINTERS
		}
		template<typename D>
		uPtr(D* dat) : data(dat) {														//Constructor de inicializacion
			INCUNIQUEPOINTERS
		}
		template<typename D>
		uPtr(uPtr<D[N]>&& other) : data(other.data) {												//Constructor de movimiento
			other.data = nullptr;
			INCUNIQUEPOINTERS
		}
		template<typename D>
		uPtr<T[N]>& operator=(D* dat) {																	//Operador de asignacion
			delete[] this->data;
			this->data = dat;
			return *this;
		}
		template<typename D>
		uPtr<T[N]>& operator=(uPtr<D[N]>&& other) {															//Operador de movimiento
			delete[] this->data;
			this->data = other.data;
			other.data = nullptr;
			return *this;
	...
	sPtr<T> make_sPtr(Args&& ...args){
		...
		ptr.data = ((typename remove_arrayness<T>::type*)(((Block_object<T>*)ptr.ctrl_block)->obj));
		return ptr;
	}
}