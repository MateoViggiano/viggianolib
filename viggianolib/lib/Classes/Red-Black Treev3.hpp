...
#define sent        ct.getV3().address()
        ...
        public:
            ...
            constexpr iterator insert(const value_type& val){
                ...
                if(root==sent){
                    ...
                    if(vcmp(val,left_most->data)){//val<left_most->data
                        left_most->left=create_node(val);
            ...
            constexpr iterator insert(value_type&& val){
                ...
                value_compare vcmp(ct.getV1());
                if(root==sent){
                    x=root=left_most=right_most=create_node(static_cast<value_type&&>(val));
                    x->right=x->left=sent;
                }
                else if(!vcmp(val,right_most->data)){//val>=right_most->data
                    if(vcmp(right_most->data,val)){//val>right_most->data
                        right_most->right=create_node(static_cast<value_type&&>(val));
                        right_most->right->prev=right_most;
                        right_most=right_most->right;
                        right_most->right=sent;
                        x=right_most;
                    ...
                    if(vcmp(val,left_most->data)){//val<left_most->data
                        left_most->left=create_node(static_cast<value_type&&>(val));
                        left_most->left->prev=left_most;
                        left_most=left_most->left;
                        left_most->left=sent;
                        x=left_most;
                    ...
                    while(true){
                        if(vcmp(val,x->data)){
                            if(x->left==nullptr){
                                x->left=create_node(static_cast<value_type&&>(val));
                                x->left->prev=x;
                                x=x->left;
                                break;							
                            }
                            ...
                            if(x->right==nullptr){
                                x->right=create_node(static_cast<value_type&&>(val));
                                x->right->prev=x;
                                x=x->right;
                                break;							
                            }
            ...
            constexpr iterator find(const key_type& val){
                ...
                else if(!cmp(val,Traits::getKey(right_most->data)) && !cmp(Traits::getKey(right_most->data),val))/*val==right_most->data*/ return iterator(right_most);
