...
namespace mpv{
	...
	namespace al{
		...
		template<typename Alloc>
		inline void memcopy(typename allocator_traits<Alloc>::pointer dest,typename allocator_traits<Alloc>::const_pointer source,typename allocator_traits<Alloc>::size_type n){
			if constexpr(!is_trivially_copy_assignable_v<typename allocator_traits<Alloc>::value_type>){
				while(n--)
					*(dest++)=*(source++);
			}
			else memcpy(dest,source,n*sizeof(typename allocator_traits<Alloc>::value_type));
		}//RECORDATORIO: memcopy y memmove asignan los valores, no construyen, por lo que en caso de que estas operaciones sean triviales no necesito tener en cuenta si el Allocator usa un constructor customizado, puedo simplemente copiarlas con memcpy
		// template<typename Alloc>
		...
		template<typename Alloc>
		inline void memmove(typename allocator_traits<Alloc>::pointer dest,typename allocator_traits<Alloc>::pointer source,typename allocator_traits<Alloc>::size_type n){
			if constexpr(!is_trivially_move_assignable_v<typename allocator_traits<Alloc>::value_type>){
				while(n--)
					*(dest++)=static_cast<typename allocator_traits<Alloc>::value_type&&>(*(source++));			
			}
			else memcpy(dest,source,n*sizeof(typename allocator_traits<Alloc>::value_type));
