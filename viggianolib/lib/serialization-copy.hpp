...
namespace mpv{
    namespace serialization{
        ...
        class Serializer:public Vector<unsigned char>{
        public:
            using Base = Vector<unsigned char>;
            using Base::Base;
            template<typename T>
            Serializer(const Serialized<T> val):Base(val.bytearray,val.bytearray+sizeof(val)){}
            Serializer(const Vector<unsigned char>& vec):Base(vec){}
            Serializer operator+(const Serializer& other)const&{
                return this->Base::operator+(other);
            }
            Serializer operator+(Serializer&& other)const&{
                return this->Base::operator+(static_cast<Serializer&&>(other));
            }
            Serializer operator+(const Serializer& other)&&{
                return static_cast<Serializer&&>(*this).Base::operator+(other);
            }
            Serializer operator+(Serializer&& other)&&{
                return static_cast<Serializer&&>(*this).Base::operator+(static_cast<Serializer&&>(other));
            }
            Serializer& operator+=(const Serializer& other){
            ...
            Serializer& operator+=(Serializer&& other){
                this->Base::operator+=(static_cast<Serializer&&>(other));
                return *this;
            }
            template<typename T>
            Serializer operator+(const Serialized<T> trivial)const{
                Serializer res(sizeof(trivial)+this->size());
                mpv::copy(res.begin(),this->begin(),this->end());
                *reinterpret_cast<Serialized<T>*>(res.get_array()+this->size())=trivial;
                return res;
            }
            template<typename T>
            Serializer& operator+=(Serialized<T> trivial){
                this->insert(this->end(),mpv::begin(trivial.bytearray),mpv::end(trivial.bytearray));
                return *this;
            }
        };
        template<typename T>
        Serializer operator+(Serialized<T> fst,const Serializer& scnd){
            Serializer res(sizeof(fst)+scnd.size());
            *reinterpret_cast<Serialized<T>*>(res.get_array())=fst;
            mpv::copy(res.begin()+sizeof(fst),scnd.begin(),scnd.end());
            return res;
        }
        template<typename T>
        constexpr enable_if_t<is_trivially_copyable_v<T>,Serializer> serialize(const T& val){
            return Serialized<T>(val);
        }
        template<typename T,typename U>
        Serializer serialize(const MapPair<T,U>& pair){
            return serialize(pair.key)+serialize(pair.val);
        }
        template<typename T>
        Serializer serialize(NullTerminatedArray<T> array){
            return Serializer(reinterpret_cast<const unsigned char*>(array.ptr),reinterpret_cast<const unsigned char*>(array.ptr)+(array.size+1)*sizeof(T));
        }
        template<typename Container>
        enable_if_t<!is_trivially_copyable_v<Container> && is_trivially_serializable_container_v<Container>,Serializer> serialize(const Container& cont){
        ...
        public:
            ...
            template<typename T>
            T read(){
                return Deserialize<T>::template deserialize<except_t,runtime_check>(data,end);
            }
            template<typename T>
        ...
        struct Deserialize<T,enable_if_t<is_trivially_copyable_v<T>>>{
            ...
            static constexpr T deserialize(const unsigned char*& data,const unsigned char* end)noexcept(runtime_check==false){
                if constexpr(runtime_check){
                    if(data+sizeof(T)>end) throw except_t();
                }
        ...
        struct Deserialize<MapPair<K,V>>{
        	template<typename except_t,bool runtime_check=true>
            static constexpr MapPair<K,V> deserialize(const unsigned char*& data,const unsigned char* end){
                K key=Deserialize<K>::template deserialize<except_t,runtime_check>(data,end);
                return MapPair<K,V>(static_cast<K&&>(key),Deserialize<V>::template deserialize<except_t,runtime_check>(data,end));
            }
        };
        ...
        struct Deserialize<Sequence,enable_if_t<!is_trivially_copyable_v<Sequence> && is_trivially_serializable_container_v<Sequence>>>{
            template<typename except_t,bool runtime_check=true>
            static constexpr Sequence deserialize(const unsigned char*& data,const unsigned char* end){
                ...
                return Sequence(rebound,rebound+size);
            }            
        };
        ...
        struct Deserialize<Sequence,enable_if_t<has_push_back_v<Sequence> && is_iterable_v<Sequence> && has_size_v<Sequence> && !is_trivially_copyable_v<Sequence> && !is_trivially_serializable_container_v<Sequence>>>{
            ...
            static constexpr Sequence deserialize(const unsigned char*& data,const unsigned char* end){
                ...
                for(size_type i=0;i<size;i++)
                    cont.push_back(Deserialize<typename Sequence::value_type>::template deserialize<except_t,runtime_check>(data,end));
                return cont;
            }
        };
        ...
        struct Deserialize<Tree,enable_if_t<has_get_root_v<Tree> && is_iterable_v<Tree> && has_size_v<Tree> && !is_trivially_copyable_v<Tree> && !is_trivially_serializable_container_v<Tree>>>{
            template<typename except_t,bool runtime_check=true>
            static constexpr Tree deserialize(const unsigned char*& data,const unsigned char* end){
                ...
                data+=sizeof(size_type);
                for(size_type i=0;i<size;i++)
                    cont.insert(Deserialize<typename Tree::value_type>::template deserialize<except_t,runtime_check>(data,end));
                return cont;
            }
        };
    }
